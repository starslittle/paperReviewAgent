...
def print_section_structure(element, indent=0, max_depth=5, current_depth=0):
    ...
    if element.tag == "Section" and current_depth < max_depth:
        ...
        if paragraphs or images or tables:
            if subsections:
                print(
                    f"{'  ' * (indent+1)}│ 📄 内容: {len(paragraphs)}段, {len(images)}图, {len(tables)}表"
                )
            else:
                print(
                    f"{'  ' * (indent+1)}└─ 📄 内容: {len(paragraphs)}段, {len(images)}图, {len(tables)}表"
    ...
    # 递归处理子Section
    if element.tag == "Document" or (
        element.tag == "Section" and current_depth < max_depth
    ):
...
if __name__ == "__main__":
    ...
    xml_path = os.path.join(_root, "sample_results", "outline_bylw-pgy.xml")

    if not os.path.exists(xml_path):
        print(f"File not found: {xml_path}")
        print(
            "Please run from project root or ensure sample_results/outline_bylw-pgy.xml exists."
        )
    else:
