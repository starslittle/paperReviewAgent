...
def main():
    ...
    parser.add_argument(
        "--data-path", type=str, default="preprocess/processed_output/MinerU/bylw-pgy"
    )
            ...
            if style == "Image":
                ...
                try:
                    if isinstance(row["para_text"], str):
                        ...
                        print(
                            f"  图片信息: {json.dumps(row['para_text'], ensure_ascii=False, indent=2)[:200]}"
                        )
                    else:
    ...
    if not has_heading:
        ...
        print("   2. 预处理步骤 (preprocess/step2_process.py) 未正确标记标题")
    ...
    print(
        f"每页元素数量: 平均 {page_stats['mean']:.1f}, 最少 {int(page_stats['min'])}, 最多 {int(page_stats['max'])}"
    )
