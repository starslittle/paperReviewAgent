...
def main():
    data_path = "preprocess/processed_output/MinerU/bylw-pgy"
    output_path = "sample_results/outline_nested_bylw-pgy.xml"
    ...
    def print_section(element, indent=0, max_depth=3, current_depth=0):
        if element.tag == "Section" and current_depth < max_depth:
            ...
            print(f"{'  ' * indent}  ├─ 段落: {len(paragraphs)}, 子章节: {len(subsections)}")
