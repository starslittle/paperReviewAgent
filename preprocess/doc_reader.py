...
def main():
    ...
    parser = argparse.ArgumentParser(description="DocReader 测试工具")
    parser.add_argument("--doc-id", type=str, required=True, help="文档ID")
    parser.add_argument("--data-dir", type=str, default="./processed_output/", help="数据目录")

    args = parser.parse_args()

    # 构建数据路径
    data_path = os.path.join(args.data_dir, args.doc_id)
    ...
    if not os.path.exists(os.path.join(data_path, "data.pkl")):
        print(f"错误: data.pkl 文件不存在: {os.path.join(data_path, 'data.pkl')}")
        return

    try:
        # 创建 DocReader 实例
        print(f"正在加载文档: {args.doc_id}")
        reader = DocReader(data_path)

        # 显示基本信息
        print(f"文档页数: {reader.num_page}")
        print(f"章节数量: {len(reader.section_dict)}")
        print(f"图片数量: {len(reader.image_path_dict)}")
        print(f"表格数量: {len(reader.table_image_path_dict)}")
        print(f"段落数量: {reader.para_count}")

        # 显示大纲
        print("\n=== 文档大纲 ===")
        outline = reader.get_outline_root()
        print(ET.tostring(outline, encoding='unicode', method='xml')[:1000] + "...")

        print("\n✅ DocReader 测试成功!")
        print(f"XML树根节点: {reader.root.tag}")
        print(f"子节点数量: {len(list(reader.root))}")
